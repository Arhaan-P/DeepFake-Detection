...
All script files are now organized under `scripts/` subdirectories. Use **PowerShell** commands only (no bash/Linux syntax).

| File                                         | Purpose                                                 |
| -------------------------------------------- | ------------------------------------------------------- |
| `scripts/run_pipeline.py`                    | End-to-end pipeline orchestrator                        |
| `scripts/preprocessing/augment_videos.py`    | 16x data augmentation                                   |
| `scripts/preprocessing/preprocess_videos.py` | MediaPipe feature extraction                            |
| `scripts/training/train.py`                  | Model training with balanced sampling                   |
| `scripts/evaluation/evaluate.py`             | Comprehensive metrics + LOOCV                           |
| `scripts/evaluation/ablation_study.py`       | Ablation: CNN-only, LSTM-only, Transformer-only, Hybrid |
| `scripts/evaluation/run_gradcam.py`          | GradCAM explainability visualizations                   |
| `scripts/inference/inference.py`             | Single video prediction                                 |
| `scripts/enrollment/enroll_identities.py`    | Build identity gait signatures                          |
...
| `models/full_pipeline.py`                    | End-to-end model assembly                               |
...
**DO NOT use bash/Linux syntax** like `./script.sh`, `source activate`, or `&&` chaining. Use PowerShell equivalents instead.
