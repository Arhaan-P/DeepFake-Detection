...
\else
  \author{\IEEEauthorblockN{Arhaan Penwala, Abhishek Arun Raja K,
          Aditya Bharti, Vibhav Sharma, and Dr.\ Deepika Roselind J}
...
Behavioural biometrics have been applied to
deepfake detection since 2019~\cite{agarwal2019}, and whole-body pose and
motion cues since at least 2024--2025~\cite{humanactionclips,ttdf,convkeypoints}.
What remains under-explored is the use of \emph{locomotion gait-cycle skeletal
dynamics as the primary discriminative feature} for verifying a claimed
identity in a face-swapped video under subject-disjoint protocol. The nearest
work~\cite{gaitfidelity} measures gait fidelity in generated animation without
building a detector; the nearest detectors~\cite{ttdf,humanactionclips} target
whole-body synthesis rather than face-swap, where the body is authentic and
only the claim is false. This is an under-explored niche, not a vacant one.
...
Each recording was expanded $16\times$ ($66 \times 16 = 1{,}056$) under the
constraint that no transformation may alter the underlying skeletal trajectory.
Photometric operations (Gaussian blur, brightness $\pm0.2$--$0.3$, contrast,
colour jitter, grayscale, Gaussian noise) change pixels but not the recovered
pose; geometric operations (horizontal flip, rotation capped at $10^{\circ}$,
zoom) stay anatomically plausible; temporal operations (speed $0.8\times$ and
$1.2\times$, reversal) exploit the approximate time-reversal symmetry and speed
variability of the gait cycle~\cite{perry_gait}. A combined multi-augmentation
pass completes the sixteen.
...
\emph{et al.}~\cite{stenum2021} validated it against a Vicon optical
motion-capture system, reporting mean absolute errors of $4.0^{\circ}$,
$5.6^{\circ}$ and $7.4^{\circ}$ for sagittal-plane hip, knee and ankle angles.
We cite that for the general claim it supports---video pose estimation agrees
with laboratory motion capture to within a few degrees---noting it evaluated
OpenPose rather than MediaPipe, so it corroborates rather than directly
evidences our backend.
...
it included. Fold ordering is stable across metrics (subjects hard by AUC are
hard by accuracy, F1 and EER alike), pointing to a property of those subjects'
gait rather than threshold artefacts, and precision and recall trade off across
folds in opposite directions (Subject 6: $95.59\%$ precision, $76.47\%$ recall;
Subject 9: $81.00\%$ precision, $95.29\%$ recall), so a single global threshold
sits at a different point on each subject's curve.
...
The left shoulder receives the highest attribution ($1.000$), followed by the
right heel ($0.940$), left foot index ($0.931$) and left knee ($0.897$). The
right shoulder ($0.873$), right ankle ($0.810$), right foot index ($0.766$),
right knee ($0.726$), left heel ($0.709$) and left ankle ($0.673$) follow, and
both hips rank lowest (left $0.665$, right $0.566$).
...
the coordinate normalisation. We do not over-interpret the left--right
asymmetry (left shoulder $1.000$ against right $0.873$; left foot $0.931$ against
right $0.766$; but right heel $0.940$ against left $0.709$): with 13 subjects and
26 samples it is as consistent with cohort-specific asymmetry or frontal/side
viewpoint bias as with anything learned about laterality.
...
Attribution decomposes by feature block as follows (Fig.~\ref{fig:groups}). Coordinates take $47.68\%$, velocities $37.40\%$ and joint angles $14.92\%$, against dimensional shares of $46.15\%$, $46.15\%$ and $7.69\%$: per-dimension ratios of $1.03$, $0.81$ and $1.94$. The six angle features are thus roughly twice as influential as the average coordinate and nearly two and a half times the average velocity. This is the clearest practical finding here: the six scale-free, rotation-invariant biomechanical constraints are the most information-dense part of the descriptor, suggesting a cheaper feature set weighted towards angular kinematics may retain most of the discriminative power---directly relevant to embedded deployment. Within the angle block the left ankle ($1.000$) and left knee ($0.896$) dominate, echoing the distal emphasis in the joint-level ranking.
...
We deliberately report no head-to-head table. The figures quoted in
Section~\ref{sec:related}---$\sim$86--89\% AUC for FTCN and 89.5\% for
AltFreezing on cross-dataset Celeb-DF, 92.5\% and 82.6\% recognition accuracy
for GaitFormer and GaitPT on CASIA-B---each use a different corpus, protocol and
task, and our $94.95\%$ is a leave-one-subject-out verification result on a
13-subject corpus collected by the authors. Tabulating them would invite a
ranking they cannot support; no claim of superiority is made. Gait evidence has
begun to appear in forensic practice: a commercial analysis was reported in 2025
as accepted evidence in a European murder case where the subject's face
occupied roughly $10 \times 5$ pixels~\cite{forensic_gait}. We cite this with
its limits stated---a single trade-press account of a case not yet tried, with a
sub-$0.1\%$ EER that is a vendor figure, not an independent benchmark. It
indicates institutional interest, not a validated forensic standard.
...
but because it is independent. \emph{Privacy}: the pipeline reduces video to 12
skeletal coordinates per frame at the first stage, retaining no facial imagery;
a smaller disclosure than a face template, though not anonymity, since gait is
itself a biometric. \emph{Degradation profile}: gait remains recoverable at
distances and resolutions where facial analysis is not
viable~\cite{forensic_gait}.
...
Priorities follow from the limitations: quantify detection on a substantially
larger set of forgeries spanning multiple face-swap models, and extend the
cohort in size and demographic range. Graph convolutional
modelling~\cite{gaitgraph}, per-subject threshold calibration (Section~\ref{sec:perfold})
and a reduced angle-weighted descriptor for embedded, real-time screening are
natural next steps.
...
the ablation ordering is established at this cohort size only. Detectors that
seek evidence inside the region an adversary controls are in a race they must
