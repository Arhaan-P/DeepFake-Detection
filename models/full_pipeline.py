...
class GaitDeepfakeDetector(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            input_dim: per-timestep gait feature dimension (78 for the
                shipped MediaPipe extractor)
            encoder_hidden_dims: channel widths of the GaitEncoder's
                residual blocks
            encoder_output_dim: GaitEncoder's per-timestep output dimension,
                also the temporal model's input dimension
            ...
            transformer_heads: number of transformer self-attention heads
            transformer_layers: number of stacked transformer encoder layers
            embedding_dim: fused gait embedding dimension (auxiliary branch
                output; see note below on the verification decision path)
            verification_hidden: hidden size used by `IdentityVerifier` and
                by `diff_conv`/`diff_classifier`
            dropout: dropout probability applied throughout the model
            use_multi_scale_encoder: use `MultiScaleGaitEncoder` instead of
                `GaitEncoder`; not the default and not used for any reported
                result
        """
        super().__init__()
        ...
        self.identity_verifier = IdentityVerifier(
            embedding_dim=embedding_dim, hidden_dim=verification_hidden, dropout=dropout
        )
    ...
    def verify_identity(
        self, video_features: torch.Tensor, claimed_features: torch.Tensor
...
class GaitDeepfakeDetectorWithTriplet(GaitDeepfakeDetector):
    ...
    def __init__(self, *args, margin: float = 1.0, **kwargs):
        """
        Args:
            *args: forwarded to `GaitDeepfakeDetector.__init__`
            margin: triplet loss margin, see `TripletLossNetwork`
            **kwargs: forwarded to `GaitDeepfakeDetector.__init__`
        """
        super().__init__(*args, **kwargs)

        self.triplet_network = TripletLossNetwork(
            embedding_dim=self.embedding_dim, margin=margin
        )
