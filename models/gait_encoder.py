...
class ResidualBlock1D(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            kernel_size: convolution kernel size (odd, so padding preserves length)
            stride: convolution stride; a value != 1 also routes the skip
                connection through a matching strided 1x1 conv
        """
        super().__init__()
...
class GaitEncoder(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            input_dim: per-timestep feature dimension (78 for the shipped
                MediaPipe extractor: 36 coords + 6 angles + 36 velocities)
            hidden_dims: channel width of each successive residual block
            output_dim: final per-timestep embedding dimension
            dropout: dropout probability applied after the input projection,
                between residual blocks, and after the output projection
        """
        super().__init__()
        ...
        self.output_proj = nn.Sequential(
            nn.Linear(hidden_dims[-1], output_dim), nn.ReLU(), nn.Dropout(dropout)
        )
...
class MultiScaleGaitEncoder(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            hidden_dim: channel width shared by every parallel branch
            output_dim: final per-timestep embedding dimension, after fusion
            kernel_sizes: convolution kernel size of each parallel branch;
                one branch is built per entry, letting each capture a
                different temporal receptive field
