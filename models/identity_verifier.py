...
class GaitComparisonNetwork(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            embedding_dim: dimension of the two input gait embeddings
            hidden_dim: projection size both embeddings are mapped to before
                comparison
    ...
    def forward(
        self, video_embedding: torch.Tensor, claimed_embedding: torch.Tensor
    ) -> torch.Tensor:
        """
...
class TripletLossNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim: int = 256, margin: float = 1.0):
        """
        Args:
            embedding_dim: dimension of anchor/positive/negative embeddings
            margin: minimum gap required between positive and negative
                distances before the triplet loss is zero
        """
        super().__init__()
    ...
    def forward(
        ...
        metrics = {
            ...
            "margin_violations": (pos_distance + self.margin > neg_distance)
            .float()
            .mean()
            .item(),
        }
...
class ContrastiveLossNetwork(nn.Module):
    ...
    def __init__(self, embedding_dim: int = 256, margin: float = 2.0):
        """
        Args:
            embedding_dim: dimension of the two input embeddings
            margin: maximum distance enforced between different-person pairs
        """
        super().__init__()
    ...
    def forward(
        self, embedding1: torch.Tensor, embedding2: torch.Tensor, labels: torch.Tensor
        ...
        metrics = {
            ...
            "same_pair_dist": (distance * labels).sum().item()
            / (labels.sum().item() + 1e-8),
            "diff_pair_dist": (distance * (1 - labels)).sum().item()
            / ((1 - labels).sum().item() + 1e-8),
        }
...
class IdentityVerifier(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            hidden_dim: projection size both embeddings are mapped to before
                comparison
            threshold: default decision threshold on P(authentic) used by
                `verify()`; the trained pipeline instead uses a Youden's-J
                threshold derived from ROC analysis, not this default
            dropout: dropout probability applied throughout the network
        """
        super().__init__()
    ...
    def forward(
        ...
        product = video_proj * claimed_proj
        combined = torch.cat(
            [video_proj, claimed_proj, diff, product], dim=1
        )  # (B, H*4)
...
if __name__ == "__main__":
    ...
    print(
        f"Verifier: prediction={result['prediction']}, similarity={result['similarity']}"
    )
