...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, max_len: int = 500, dropout: float = 0.1):
        """
        Args:
            d_model: embedding dimension the encoding is added to
            max_len: longest sequence length the precomputed table supports
...
class BiLSTMEncoder(nn.Module):
    ...
    def __init__(
        ...
        Args:
            ...
            dropout: inter-layer dropout (only applied when num_layers > 1)
            bidirectional: whether to run forward and backward passes
        """
        super().__init__()
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        Args:
            input_dim: per-timestep input feature dimension
            d_model: transformer embedding dimension (input is projected to
                this size when input_dim differs from it)
            nhead: number of self-attention heads
            num_layers: number of stacked encoder layers
            dim_feedforward: hidden size of each layer's feedforward block
            dropout: dropout probability throughout the encoder
            max_len: longest sequence length the positional encoding supports
        """
        super().__init__()
...
class DualPathTemporalModel(nn.Module):
    ...
    def __init__(
        ...
        """
        Args:
            input_dim: per-timestep feature dimension shared by both paths
            ...
            output_dim: fused embedding dimension returned by this module
            dropout: dropout probability applied throughout both paths and
                the fusion MLP
        """
        super().__init__()
    ...
    def forward(self, x: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if lstm_output.size(-1) != trans_output.size(-1):
            lstm_output = F.pad(
                lstm_output, (0, trans_output.size(-1) - lstm_output.size(-1))
            )
...
class TemporalAttentionPool(nn.Module):
    ...
    def __init__(self, input_dim: int):
        """
        Args:
            input_dim: per-timestep feature dimension to pool over
        """
        super().__init__()
...
if __name__ == "__main__":
    ...
    print(
        f"Transformer: {x.shape} -> outputs: {outputs.shape}, embedding: {embedding.shape}"
    )
    ...
    print(
        f"AttentionPool: {x.shape} -> pooled: {pooled.shape}, weights: {weights.shape}"
    )
