...
% diagrams/GEMINI_PROMPTS.md. Until those PNGs are dropped into figures/, the
% \concept macro renders a labelled placeholder box so the document still
...
small to be a powered claim. We further report, rather than conceal, a
negative methodological result: the four variants of our
architectural ablation share an identical decision path, so their $1.58$-point
accuracy spread measures training variance and not component contribution. We
...
\item \textbf{An explicit negative result.} Our architectural ablation does not
      support the conclusion it was designed to test, because all four variants
      share an identical decision path. We report this, quantify it, and
      identify the corrected experiment (Sections~\ref{sec:ablation-results}
      and~\ref{sec:threats}).
\end{enumerate}
...
scales~\cite{lecun_efficient}. Critically, $\mu_j$ and $\sigma_j$ are estimated
on the training split alone and applied unchanged to held-out data; under LOOCV
they are recomputed inside every fold.
...
difference-based verification network. They are not evidence for or against the
CNN--BiLSTM--Transformer stack, which the present experiments do not test.
Fig.~\ref{fig:architecture} draws the separation explicitly.
...
Four variants---CNN-only, LSTM-only, Transformer-only and the full model---were
trained on identical splits with identical seeds and schedules, differing only
in the representation branch instantiated. Section~\ref{sec:ablation-results}
reports both what was measured and why the comparison does not answer the
question it was posed to answer.
...
Table~\ref{tab:ablation} and Fig.~\ref{fig:ablation} report the four variants.
The measured spread is narrow: $88.93\%$ to $90.51\%$ accuracy, a range of
$1.58$ points, with Transformer-only nominally highest in accuracy, F1, AUC and
precision, and the full model highest in recall.

These numbers do not support conclusions about component contribution, and we
do not draw any. On inspection of the released implementation, all four
variants compute their logits through an identical
\texttt{diff\_conv} $\rightarrow$ \texttt{diff\_classifier} head applied to the
same raw pairwise comparison tensor; the CNN, BiLSTM and Transformer branches
that nominally distinguish the variants produce only the auxiliary embedding of
Section~\ref{sec:aux} and never reach the logits. Instantiating each variant
and counting confirms it: every one carries exactly $133{,}058$ parameters on
the decision path, differing only in the $74{,}000$ to $716{,}000$ parameters
that sit off it.

The four variants are therefore the same classifier trained four times, and the
$1.58$-point spread measures run-to-run variance under different random
initialisation of surrounding modules---which is itself a useful calibration,
since it bounds the difference that can be considered meaningful in
Table~\ref{tab:ablation} at somewhat under two accuracy points. The experiment
that would answer the original question requires routing each branch's output
into the classifier; Section~\ref{sec:threats} states it as required future
work rather than as a completed contribution.

\begin{table}[!t]
\caption{Architectural Ablation (Single Split). All Variants Share an
Identical Decision Path; See Section~\ref{sec:ablation-results}}
\label{tab:ablation}
\centering
\small
\begin{tabular}{@{}lrrrrrr@{}}
\toprule
\tabhead{Variant} & \tabhead{Acc} & \tabhead{F1} & \tabhead{AUC} &
\tabhead{Total} & \tabhead{Decision} \\
 & \tabhead{(\%)} & \tabhead{(\%)} & \tabhead{(\%)} &
\tabhead{params} & \tabhead{path} \\
\midrule
CNN-only         & 88.93 & 89.02 & 96.40 & 262{,}658 & 133{,}058 \\
LSTM-only        & 89.33 & 89.11 & 96.10 & 207{,}042 & 133{,}058 \\
Transformer-only & 90.51 & 90.24 & \textbf{96.89} & 539{,}970 & 133{,}058 \\
Full model       & 90.32 & \textbf{90.37} & 96.11 & 848{,}614 & 133{,}058 \\
\midrule
Spread           & 1.58 & 1.35 & 0.79 & --- & 0 \\
\bottomrule
\end{tabular}
\end{table}
...
\caption{Architectural ablation over the four trained variants. (a) Measured
validation metrics; all four fall within $1.58$ accuracy points of one another.
(b) The reason: parameter counts measured by instantiating each variant show
that all four carry exactly the same $133{,}058$ parameters on the verification
decision path, and differ only in parameters that never reach the logits. The
variants are one classifier trained four times, so panel (a) is best read as an
estimate of run-to-run variance---roughly two accuracy points---rather than as
evidence of component contribution.}
\label{fig:ablation}
\end{figure*}
...
\emph{The ablation does not test what it was designed to test.} As established
in Section~\ref{sec:ablation-results}, the four variants share a decision path
and the reported spread is run-to-run variance. The paper accordingly makes no
claim about the contribution of the CNN, BiLSTM or Transformer components, and
the architecture in Table~\ref{tab:config} should be understood as a
difference-based temporal CNN with an unused representation branch attached.
The corrected experiment---routing each branch's embedding into the classifier
and retraining---is required before any architectural claim can be made.
...
We have also been explicit about what the experiments do not show. The
architectural ablation, as implemented, compares four instances of the same
classifier and cannot speak to component contribution; the face-swap
validation, though it correctly rejected all three generated clips with
similarity to the true body source above $0.9998$ in every case, remains an
existence check on a single generator rather than a statistically powered
claim; and thirteen subjects is a small basis for a generalisation claim.
These are stated as they are so that the result which does stand---that
skeletal gait discriminates claimed identity in subject-disjoint evaluation at
$94.95\%$ pooled AUC---can be read for what it is.
