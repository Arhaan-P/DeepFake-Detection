...
def extract_identity_from_filename(filename: str) -> str:
    """Extract identity from video filename (e.g., 'SubjectA_F.mp4' -> 'SubjectA')."""
...
def enroll_from_videos(
    videos_dir: str, output_file: str, sequence_length: int = 60, min_videos: int = 1
) -> dict:
    ...
    for identity, videos in tqdm(
        sorted(videos_by_identity.items()), desc="Enrolling identities"
    ):
        print(f"\n{'='*40}")
        ...
        if valid_videos >= min_videos:
            ...
            print(
                f"  ✗ Not enough valid videos (need {min_videos}, got {valid_videos})"
            )
...
def enroll_from_features(features_file: str, output_file: str) -> dict:
    ...
    for video_path, features in all_features.items():
        identity = features.get("identity") or extract_identity_from_filename(
            video_path
        )
    ...
    for identity, feature_list in sorted(features_by_identity.items()):
        ...
        avg_joint_angles = np.mean([gf["joint_angles"] for gf in gait_features], axis=0)
        avg_velocities = np.mean([gf["velocities"] for gf in gait_features], axis=0)
        avg_accelerations = np.mean(
...
def main():
    ...
    # Mode selection
    parser.add_argument(
        "--from_videos", action="store_true", help="Enroll directly from video files"
    )
    parser.add_argument(
        "--from_features",
        action="store_true",
    ...
    if args.from_videos:
        ...
        enroll_from_videos(
            args.videos_dir, args.output, args.sequence_length, args.min_videos
        )
