...
class LSTMEncoder(nn.Module):
    ...
    def __init__(self, input_dim=78, hidden=64, layers=1, dropout=0.1):
        ...
        self.enc = BiLSTMEncoder(
            input_dim=input_dim, hidden_dim=hidden, num_layers=layers, dropout=dropout
        )
...
class EncodedDiffVerifier(nn.Module):
    ...
    def __init__(
        self, encoder, verification_hidden=64, dropout=0.1, fuse_raw=False, raw_dim=78
    ):
        super().__init__()
...
def run_one(spec, train_loader, test_loader, device, epochs, lr, dropout, seed):
    ...
    model = EncodedDiffVerifier(
        encoder_fn(dropout), dropout=dropout, fuse_raw=fuse_raw
    ).to(device)
...
def main():
    ...
    ap.add_argument(
        "--enrolled_file", default="data/gait_features/enrolled_identities.pkl"
    )
    ...
    print(
        f"  Runs/fold    : {len(work)}   (incl. 1 legacy-normalization arm)", flush=True
    )
    ...
    for fold_idx, test_person in enumerate(persons, 1):
        ...
        print("-" * 78, flush=True)
        print(
            f"  FOLD {fold_idx}/{len(persons)}  --  held out: {test_person}", flush=True
        )
        print("-" * 78, flush=True)
        ...
        for variant, seed, norm in work:
            ...
            print(
                f"    [{done:3d}/{total_runs}] {variant:<24s} "
    ...
    print("\n" + "=" * 78, flush=True)
    print(
        "  FINAL RESULTS  (mean +/- std across folds; pooled over all scores)",
        flush=True,
    )
    print("=" * 78, flush=True)
    hdr = (
        f'  {"Variant":<24s} {"seed":>4s} {"norm":>6s} {"params":>9s} '
    ...
    for k, agg in snapshot["results"].items():
        a = agg["aggregate"]
        print(
            f'  {agg["variant"]:<24s} {agg["seed"]:>4d} {agg["norm"]:>6s} '
            f'{a["params"]:>9,d} '
            ...
            f'{agg["pooled"]["roc_auc"]*100:>10.2f}',
            flush=True,
        )
    ...
    print(
        f"  Total time   : {timedelta(seconds=int(time.time() - t_start))}", flush=True
    )
    ...
    out = {
        "_meta": {
            ...
            "normalization_arms": {
                ...
                "legacy": (
                    "test set z-scored with its own statistics -- the "
