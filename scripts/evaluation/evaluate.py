...
def evaluate_verification(model, dataloader, device):
    ...
    results_per_identity = defaultdict(
        lambda: {"correct": 0, "total": 0, "similarities": []}
    )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader, desc="Evaluating")):
            ...
            for i, (true_id, claimed_id) in enumerate(
                zip(true_identities, claimed_identities)
            ):
...
def run_loocv(
    ...
    for fold_idx, test_person in enumerate(persons):
        ...
        print(f"\n{'─'*50}")
        print(
            f"Fold {fold_idx + 1}/{n_folds} — Test: {test_person}, Train: {train_persons}"
        )
        print(f"{'─'*50}")
        ...
        fm["accuracy"] = accuracy_score(
            results["labels"], results["binary_predictions"]
        )
        fm["f1"] = f1_score(
            results["labels"], results["binary_predictions"], zero_division=0
        )
        fm["precision"] = precision_score(
            results["labels"], results["binary_predictions"], zero_division=0
        )
        fm["recall"] = recall_score(
            results["labels"], results["binary_predictions"], zero_division=0
        )
        ...
        print(
            f"  Acc={fm['accuracy']:.4f}  F1={fm['f1']:.4f}  "
    ...
    if len(np.unique(all_fold_labels)) > 1:
        aggregate["pooled_roc_auc"] = float(
            roc_auc_score(all_fold_labels, all_fold_scores)
        )
    ...
    print(
        f"Precision:  {aggregate['precision']:.4f} ± {aggregate['precision_std']:.4f}"
    )
    ...
    for fm in fold_metrics:
        print(
            f"{fm['test_person']:<12} {fm['accuracy']:>7.4f} {fm['f1']:>7.4f} "
...
def save_plots(metrics, results, output_dir):
    ...
    plot_roc_curve(results["labels"], results["similarities"], save_path=roc_path)
    print(f"Saved: {roc_path}")

    # Precision-Recall Curve
    pr_path = os.path.join(output_dir, "precision_recall_curve.png")
    plot_precision_recall_curve(
        results["labels"], results["similarities"], save_path=pr_path
    )
...
def save_results(metrics, results, output_dir):
    ...
    summary = {
        ...
        "per_identity_accuracy": {
            k: float(v) for k, v in metrics["per_identity_accuracy"].items()
        },
    }
...
def main():
    ...
    parser.add_argument(
        "--test_only", action="store_true", help="Evaluate only on test set (default)"
    )
    parser.add_argument(
        "--eval_all", action="store_true", help="Evaluate on all splits"
    )
    parser.add_argument(
        "--loocv",
        action="store_true",
    ...
    parser.add_argument(
        "--lr", type=float, default=1e-4, help="Learning rate for LOOCV training"
    )
    ...
    parser.add_argument(
        "--save_results", action="store_true", help="Save evaluation results to files"
    )
