...
def plot_confusion_matrix(labels, scores, threshold=0.5):
    ...
    axes[1].set_title(
        "(b) Confusion Matrix (Normalized %)", fontsize=15, fontweight="bold"
    )
...
def plot_four_cases(cm_stats, labels, scores, threshold=0.5):
    ...
    fig.suptitle(
        f"Classification Outcomes — Pooled LOOCV (N={total}, Accuracy={accuracy:.1f}%)",
        fontsize=17,
        fontweight="bold",
        y=1.02,
    )
...
def plot_roc_curve(labels, scores):
    ...
    # Mark EER point
    ax.plot(fpr[eer_idx], tpr[eer_idx], "g^", markersize=10, label=f"EER = {eer:.4f}")
...
def plot_pr_curve(labels, scores):
    ...
    ax.plot(
        recall, precision, "b-", linewidth=2.5, label=f"PR Curve (AP = {pr_auc:.4f})"
    )
...
def plot_similarity_distribution(labels, scores, threshold=0.5):
    ...
    ax.set_title(
        "Distribution of Gait Similarity Scores", fontsize=15, fontweight="bold"
    )
...
def plot_loocv_per_fold(loocv_data):
    ...
    ax.axhline(
        y=agg["accuracy"] * 100, color="#3498db", linestyle="--", alpha=0.5, linewidth=1
    )
    ax.axhline(
        y=agg["f1"] * 100, color="#2ecc71", linestyle="--", alpha=0.5, linewidth=1
    )
    ax.axhline(
        y=agg["roc_auc"] * 100, color="#e74c3c", linestyle="--", alpha=0.5, linewidth=1
    )

    ax.set_xlabel("Test Subject (Held-Out Fold)", fontsize=14)
    ax.set_ylabel("Score (%)", fontsize=14)
    ax.set_title(
        "Leave-One-Out Cross-Validation: Per-Fold Results",
        fontsize=15,
        fontweight="bold",
    )
    ax.set_xticks(x)
...
def plot_ablation_study(ablation_data):
    ...
    ax.set_ylabel("Score (%)", fontsize=14)
    ax.set_title(
        "Ablation Study: Model Architecture Comparison", fontsize=15, fontweight="bold"
    )
    ...
    ax.text(
        0.02,
        0.02,
        "Numbers below bars: parameter count",
        transform=ax.transAxes,
        fontsize=9,
        color="#888888",
    )
...
def plot_gradcam_importance(gradcam_data):
    ...
    axes[0].barh(
        range(len(sorted_names)), sorted_values, color=sorted_colors, alpha=0.85
    )
...
def generate_tables(
    ...
    rows = [
        (
            "Accuracy (%)",
            f"{agg['accuracy']*100:.2f}",
            f"± {agg['accuracy_std']*100:.2f}",
        ),
        (
            "Precision (%)",
            f"{agg['precision']*100:.2f}",
            f"± {agg['precision_std']*100:.2f}",
        ),
    ...
    total = cm_stats["TP"] + cm_stats["TN"] + cm_stats["FP"] + cm_stats["FN"]
    tables_text.append(
        f"{'Outcome':<25} {'Count':<10} {'Percentage':<15} {'Description':<40}"
    )
    tables_text.append("-" * 90)
    tables_text.append(
        f"{'True Positive (TP)':<25} {cm_stats['TP']:<10} {cm_stats['TP']/total*100:.1f}%{'':<10} {'Authentic correctly classified':<40}"
    )
    tables_text.append(
        f"{'True Negative (TN)':<25} {cm_stats['TN']:<10} {cm_stats['TN']/total*100:.1f}%{'':<10} {'Deepfake correctly classified':<40}"
    )
    tables_text.append(
        f"{'False Positive (FP)':<25} {cm_stats['FP']:<10} {cm_stats['FP']/total*100:.1f}%{'':<10} {'Deepfake misclassified as authentic':<40}"
    )
    tables_text.append(
        f"{'False Negative (FN)':<25} {cm_stats['FN']:<10} {cm_stats['FN']/total*100:.1f}%{'':<10} {'Authentic misclassified as deepfake':<40}"
    )
    ...
    tables_text.append("=" * 70)
    tables_text.append(
        f"{'Subject':<12} {'Accuracy':>10} {'F1':>10} {'Precision':>10} {'Recall':>10} {'AUC':>10} {'EER':>10} {'N':>6}"
    )
    ...
    if ablation_data:
        ...
        tables_text.append("=" * 70)
        tables_text.append(
            f"{'Variant':<25} {'Params':>8} {'Acc%':>8} {'F1%':>8} {'Prec%':>8} {'Rec%':>8} {'AUC%':>8}"
        )
    ...
    if gradcam_data:
        ...
        joints_sorted = sorted(
            gradcam_data["joint_importance"].items(), key=lambda x: x[1], reverse=True
        )
...
def main():
    ...
    print(
        f"  LOOCV: {len(labels)} pooled samples, {loocv_data['aggregate']['n_folds']} folds"
    )
    if ablation_data:
    ...
    print(
        f"    TP={cm_stats['TP']}, TN={cm_stats['TN']}, FP={cm_stats['FP']}, FN={cm_stats['FN']}"
    )
    ...
    print(
        f"    AUC={roc_auc:.4f}, EER={eer:.4f}, Optimal Threshold={optimal_thresh:.4f}"
    )
    ...
    print("\n[4/4] Summary")
    print("=" * 60)
    print("Total figures generated: 10")
    print("Tables generated: 5")
