...
Usage:
    ...
    python scripts/evaluation/run_gradcam.py --video data/videos/SubjectA_S1.mp4 --claimed_identity SubjectB
...
def load_enrolled_features(enrolled_file, person_name, feature_stats=None, seq_len=60):
    ...
    if person_name not in enrolled:
        raise ValueError(
            f"Identity '{person_name}' not found. Available: {list(enrolled.keys())}"
        )
    ...
    if features is None:
        raise ValueError(
            f"Could not extract features for '{person_name}' from enrolled data"
        )
...
def run_aggregate_analysis(
    model, enrolled_file, features_file, feature_stats, device, save_dir, n_samples=20
):
    ...
    for video_key, video_data in all_features.items():
        # Extract person name from video path (e.g., 'data/videos/SubjectA_F1.mp4' → 'SubjectA')
        basename = Path(video_key).stem  # 'SubjectA_F1'
    ...
    def pose_to_78dim(pose_seq):
        ...
        coords = gait_features["normalized_coords"].reshape(
            pose_seq.shape[0], -1
        )  # (T, 36)
        angles = gait_features["joint_angles"]  # (T, 6)
        velocities = gait_features["velocities"].reshape(
            pose_seq.shape[0], -1
        )  # (T, 36)
    ...
    for person in persons:
        ...
        for vid_key, vid_data in videos[:n_per_person]:
            try:
                ...
                if feature_stats:
                    video_feat = (video_feat - feature_stats["mean"]) / (
                ...
                claimed_feat = load_enrolled_features(
                    enrolled_file, person, feature_stats
                )
    ...
    avg_group = {
        k: np.mean([g[k] for g in all_group_imp]) for k in all_group_imp[0].keys()
    }
    avg_temporal = np.mean(all_temporal_imp, axis=0)

    # Normalize
    if avg_joint.max() > 0:
        avg_joint /= avg_joint.max()
    if avg_angle.max() > 0:
        avg_angle /= avg_angle.max()
    if avg_temporal.max() > 0:
        avg_temporal /= avg_temporal.max()
    ...
    results = {
        "joint_importance": {
            GAIT_KEYPOINT_NAMES[i]: float(avg_joint[i]) for i in range(12)
        },
        "angle_importance": {
            JOINT_ANGLE_NAMES[i]: float(avg_angle[i]) for i in range(6)
        },
...
def main():
    ...
    parser.add_argument(
        "--video", type=str, default=None, help="Video file for single analysis"
    )
    parser.add_argument(
        "--claimed_identity",
        type=str,
