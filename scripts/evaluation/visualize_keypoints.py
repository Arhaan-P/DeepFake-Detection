...
Usage:
    ...
    python scripts/evaluation/visualize_keypoints.py --subjects SubjectA,SubjectB,SubjectC
...
GAIT_BONE_CONNECTIONS = [
    (0, 1),  # Shoulders
    (0, 2),
    (1, 3),  # Torso (shoulder→hip)
    (2, 3),  # Hips
    (2, 4),
    (4, 6),
    (6, 10),  # Left: hip→knee→ankle→foot
    (3, 5),
    (5, 7),
    (7, 11),  # Right: hip→knee→ankle→foot
    (6, 8),  # Left: ankle→heel
...
CANONICAL_SKELETON = {
    "L_Shoulder": (0.60, 0.25),
    "R_Shoulder": (0.40, 0.25),
    "L_Hip": (0.57, 0.50),
    "R_Hip": (0.43, 0.50),
    "L_Knee": (0.60, 0.68),
    "R_Knee": (0.40, 0.68),
...
def load_enrolled_features(enrolled_file, person_name, feature_stats=None, seq_len=60):
    ...
    if person_name not in enrolled:
        raise ValueError(
            f"'{person_name}' not found. Available: {list(enrolled.keys())}"
        )
    ...
    if isinstance(data, dict):
        ...
        if features is None and "avg_normalized_coords" in data:
            coords = data["avg_normalized_coords"].reshape(
                data["avg_normalized_coords"].shape[0], -1
            )
            angles = data["avg_joint_angles"]
            velocities = data["avg_velocities"].reshape(
                data["avg_velocities"].shape[0], -1
            )
            features = np.concatenate([coords, angles, velocities], axis=1).astype(
                np.float64
            )
    else:
        features = data
        ...
        for ax_idx in range(len(axes)):
            if ax_idx < n_display:
                ...
                axes[ax_idx].set_title(
                    f"Frame {fidx} ({time_s:.2f}s)", fontsize=10, fontweight="bold"
                )
            axes[ax_idx].axis("off")

        fig.suptitle(
            f"Gait Skeleton Overlay — {subject} (Side View)",
            fontsize=14,
            fontweight="bold",
            y=1.02,
        )
...
def generate_auth_vs_fake(
    ...
    for subject_a, subject_b in subject_pairs:
        ...
        if len(frames_data) < 2:
            print("  [SKIP] Not enough frames with poses")
            continue
        ...
        # Legend
        auth_patch = mpatches.Patch(
            color="#2ecc71", label=f"Claimed: {subject_a} (own identity)"
        )
        fake_patch = mpatches.Patch(
            color="#e74c3c", label=f"Claimed: {subject_b} (wrong identity)"
        )
        fig.legend(
            handles=[auth_patch, fake_patch],
            loc="lower center",
            ncol=2,
            fontsize=11,
            frameon=True,
        ...
        save_path = os.path.join(
            output_dir, f"auth_vs_fake_{subject_a}_vs_{subject_b}.png"
        )
        fig.savefig(save_path, dpi=300, bbox_inches="tight", facecolor="white")
        plt.close(fig)
        print(f"    Saved: {save_path}")
...
def generate_angle_timeseries(subjects, features_file, output_dir):
    ...
    fig.suptitle(
        "Joint Angle Trajectories Across Subjects",
        fontsize=14,
        fontweight="bold",
        y=1.02,
    )
...
def generate_embedding_tsne(features_file, model, feature_stats, device, output_dir):
    """
    ...
    for key in sorted(all_features.keys()):
        ...
        # Augmented videos have names like SubjectA_S1_speed_0.8 or contain 'aug'
        if "_aug" in stem.lower():
            continue
        # Original videos are {Name}_{View} e.g. SubjectA_S1, SubjectB_F2
        parts = stem.split("_")
        ...
        try:
            ...
            feat_78 = np.concatenate([coords, angles, velocities], axis=1).astype(
                np.float32
            )
            ...
            if feature_stats is not None:
                feat_tensor = (feat_tensor - feature_stats["mean"]) / (
                    feature_stats["std"] + 1e-8
                )
    ...
    print(
        f"  Computing t-SNE for {len(embeddings)} samples, "
    ...
    ax.set_title(
        "Gait Embedding Space — Inter-Subject Separability",
        fontsize=14,
        fontweight="bold",
    )
    ax.legend(
        bbox_to_anchor=(1.05, 1), loc="upper left", fontsize=9, frameon=True, ncol=1
    )
...
def generate_importance_skeleton(
    features_file, enrolled_file, model, feature_stats, device, output_dir, n_samples=20
):
    """
    ...
    for person_name, video_keys in person_videos.items():
        ...
        for vk in video_keys[:2]:  # Max 2 videos per person
            ...
            try:
                ...
                claimed_feat = load_enrolled_features(
                    enrolled_file, person_name, feature_stats
                )
                video_feat_dev = video_feat.to(device)
    ...
    ax.set_title(
        "Joint Importance for Deepfake Detection\n" "(Aggregated Gradient Attribution)",
        fontsize=14,
        fontweight="bold",
        pad=20,
    )
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Comma-separated list of subjects " "(default: all enrolled identities)",
    )
    parser.add_argument(
        "--figures",
        type=str,
    ...
    if args.subjects:
        ...
        with open(args.enrolled_file, "rb") as f:
            subjects = list(pickle.load(f).keys())
    ...
    if 4 in figures:
        generate_embedding_tsne(
            args.features_file, model, feature_stats, device, args.output_dir
        )

    if 5 in figures:
        generate_importance_skeleton(
            args.features_file,
            args.enrolled_file,
            model,
            feature_stats,
