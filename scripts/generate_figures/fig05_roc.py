...
def main():
    ...
    ax1.plot(
        fpr, tpr, color=fs.C_AUTH, lw=1.6, label="Pooled ROC (AUC = %.4f)" % pooled_auc
    )
    ax1.fill_between(fpr, tpr, alpha=0.10, color=fs.C_AUTH, lw=0)

    # Equal-error line and its intersection.
    ax1.plot([0, 1], [1, 0], ls=(0, (1, 2)), lw=0.8, color=fs.C_NEUTRAL, alpha=0.8)
    ax1.scatter(
        [eer],
        [1 - eer],
        s=26,
    ...
    ax1.annotate(
        "TPR %.2f%%\nFPR %.2f%%\n$J$ = %.4f" % (100 * tpr_j, 100 * fpr_j, jstat),
        xy=(fpr_j, tpr_j),
        xytext=(fpr_j + 0.20, tpr_j - 0.26),
        fontsize=6.8,
        color=fs.C_NEUTRAL,
        ha="left",
    ...
    ax2.plot([], [], color=fs.C_FOLD, lw=0.8, label="Remaining 11 folds")
    ax2.set_xlabel("False positive rate")
    ax2.set_ylabel("True positive rate")
    ax2.set_title(
        "(b) Per-subject folds: %.4f $\\pm$ %.4f AUC" % (aucs.mean(), aucs.std()),
        loc="left",
    )
    ...
    print(
        "    Youden tau*     %.4f (TPR %.4f, FPR %.4f, J %.4f)"
