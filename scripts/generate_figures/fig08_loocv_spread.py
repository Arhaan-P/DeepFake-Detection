...
def main():
    ...
    def draw(axis, entries, positions, colour):
        ...
        vp = axis.violinplot(vals, positions=positions, showextrema=False, widths=0.82)
        ...
        axis.boxplot(
            ...
            boxprops=dict(facecolor="white", edgecolor=fs.C_NEUTRAL, lw=0.7),
        ...
        for p, v in zip(positions, vals):
            axis.scatter(
                p + rng.uniform(-0.07, 0.07, v.size),
                v,
                s=8,
                color=colour,
                alpha=0.85,
    ...
    ax1.set_xticks(
        pos_hi + pos_lo, labels=[lab for _, lab in METRICS], rotation=30, ha="right"
    )
    ...
    for i, (a, c) in enumerate(zip(auc_sorted, acc_sorted)):
        ax2.text(
            a + 0.7, i + 0.19, "%.1f" % a, va="center", fontsize=6.2, color=fs.C_NEUTRAL
        )
        ax2.text(
            c + 0.7, i - 0.19, "%.1f" % c, va="center", fontsize=6.2, color=fs.C_NEUTRAL
        )
    ...
    for k, lab in METRICS:
        v = data[k]
        print(
            "    %-10s mean %6.2f  std %5.2f  min %6.2f (%s)  max %6.2f (%s)"
            % (
                lab,
                v.mean(),
                v.std(),
                v.min(),
                names[int(np.argmin(v))],
                v.max(),
                names[int(np.argmax(v))],
            )
        )
