...
def main():
    ...
    # ---------------- (a) skeleton overlay ----------------
    for a, b in fs.BONES:
        ax1.plot([xy[a, 0], xy[b, 0]], [xy[a, 1], xy[b, 1]],
                 color="#b8b8b8", lw=1.8, zorder=1, solid_capstyle="round")

    # Glow halo scaled by attribution, then the marker itself.
    ax1.scatter(xy[:, 0], xy[:, 1], s=110 + 1000 * (vals - vals.min()),
                c=[cmap(norm(v)) for v in vals], alpha=0.20, linewidth=0,
                zorder=2)
    ax1.scatter(xy[:, 0], xy[:, 1], s=120, c=vals, cmap=fs.SEQ_CMAP,
                norm=norm, edgecolor=fs.C_NEUTRAL, linewidth=0.7, zorder=3)

    order = np.argsort(-vals)
    rank = {int(k): r + 1 for r, k in enumerate(order)}
    for i in order[:3]:
        ax1.text(xy[i, 0], xy[i, 1], str(rank[int(i)]), ha="center",
    ...
    for side in (1, -1):
        idx = [i for i in range(len(fs.JOINTS))
               if (1 if xy[i, 0] >= 0 else -1) == side]
        ...
        for i, ly in zip(idx, ys):
            ax1.annotate("%s  %.2f" % (fs.JOINT_LABEL[fs.JOINTS[i]], vals[i]),
                         xy=(xy[i, 0], xy[i, 1]),
                         xytext=(label_x[side], ly),
                         ha="left" if side > 0 else "right", va="center",
                         fontsize=6.2, color=fs.C_NEUTRAL,
    ...
    ax1.axis("off")
    ax1.set_title("(a) Attribution on the mean enrolled skeleton", loc="left",
                  pad=8)
    ax1.text(0.5, -0.01, "viewer-facing: subject's left appears at right",
             transform=ax1.transAxes, ha="center", va="top", fontsize=6,
             color="#8c8c8c")

    cb = fig.colorbar(cm.ScalarMappable(norm=norm, cmap=fs.SEQ_CMAP), ax=ax1,
                      orientation="horizontal", fraction=0.045, pad=0.045,
                      aspect=28)
    ...
    ax2.barh(ypos, sorted_vals, height=0.68,
             color=[cmap(norm(v)) for v in sorted_vals],
             edgecolor=fs.C_NEUTRAL, linewidth=0.4)
    for i, v in enumerate(sorted_vals):
        ax2.text(v + 0.012, i, "%.3f" % v, va="center", fontsize=6.4,
                 color=fs.C_NEUTRAL)
