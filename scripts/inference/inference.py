...
Usage:
    ...
    python scripts/inference/inference.py --video path/to/video.mp4 --claimed_identity "SubjectA" --visualize
...
def load_model(checkpoint_path: str, device: torch.device):
    ...
    if feature_stats is not None:
        ...
        print(
            f"Loaded feature normalization stats (dim={feature_stats['mean'].shape[0]})"
        )
    else:
...
def load_enrolled_identity(
    ...
    if person_name not in enrolled_identities:
        ...
        raise ValueError(
            f"Person '{person_name}' not enrolled. " f"Available: {available}"
        )
    ...
    if feature_stats is not None:
        features_tensor = (features_tensor - feature_stats["mean"]) / feature_stats[
            "std"
        ]
...
def visualize_result(
    video_path: str, result: dict, extraction_result: dict, output_path: str = None
):
    ...
    if len(pose_sequence) > 1:
        ...
        movement_magnitude = np.concatenate(
            [[movement_magnitude[0]], movement_magnitude]
        )
    ...
    if len(pose_sequence) != len(frames):
        ...
        pose_interp = np.zeros(
            (len(frames), pose_sequence.shape[1], pose_sequence.shape[2])
        )
        for i in range(pose_sequence.shape[1]):
            for j in range(pose_sequence.shape[2]):
                pose_interp[:, i, j] = np.interp(
                    new_indices, old_indices, pose_sequence[:, i, j]
                )
...
def print_result(
    result: dict, claimed_identity: str, video_path: str, identity_check: dict = None
):
    ...
    if result["is_authentic"]:
        ...
        print(
            f"Actual:     Gait matches {GREEN}{actual}{RESET} "
            f"(similarity: {identity_check['best_similarity']:.4f})"
        )
        print(
            f"Claimed:    {claimed_identity} "
        ...
        sorted_scores = sorted(
            identity_check["scores"].items(), key=lambda x: x[1], reverse=True
        )
        print("\nAll identity scores:")
        ...
        print("Status:     Gait matches NO enrolled identity")
        print(
            f"Claimed:    {claimed_identity} "
            f"(similarity: {result['similarity']:.4f})"
        )
...
def main():
    ...
    parser.add_argument(
        "--visualize", action="store_true", help="Visualize attention on video"
    )
    parser.add_argument(
        "--output", type=str, default=None, help="Path to save visualization"
    )
    ...
    claimed_features = load_enrolled_identity(
        args.enrolled_file, args.claimed_identity, device, feature_stats=feature_stats
    )

    # Extract video features (with same normalization as training)
    video_features, extraction_result = extract_video_features(
        args.video, args.sequence_length, device, feature_stats=feature_stats
    )
