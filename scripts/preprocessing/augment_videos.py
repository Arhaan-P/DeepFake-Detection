...
class VideoAugmenter:
        ...
        configs = {
                ...
                "spatial": A.Compose(
                    [
                        A.RandomBrightnessContrast(
                            brightness_limit=(0.2, 0.3), contrast_limit=0, p=1.0
                        ),
                    ]
                ),
                "temporal": None,
            ...
            "bright_down": {
                "spatial": A.Compose(
                    [
                        A.RandomBrightnessContrast(
                            brightness_limit=(-0.3, -0.2), contrast_limit=0, p=1.0
                        ),
                    ]
                ),
                "temporal": None,
            ...
            "contrast_up": {
                "spatial": A.Compose(
                    [
                        A.RandomBrightnessContrast(
                            brightness_limit=0, contrast_limit=(0.2, 0.3), p=1.0
                        ),
                    ]
                ),
                "temporal": None,
            ...
            "rotate_left": {
                "spatial": A.Compose(
                    [
                        A.Rotate(
                            limit=(-10, -5), p=1.0, border_mode=cv2.BORDER_REFLECT
                        ),
                    ]
                ),
                "temporal": None,
    ...
    def save_video(
        self, frames: np.ndarray, output_path: str, fps: float, width: int, height: int
    ):
    ...
    def apply_spatial_augmentation(
        self, frames: np.ndarray, transform: A.Compose
    ) -> np.ndarray:
        """
    ...
    def apply_temporal_speed_change(
        self, frames: np.ndarray, speed_factor: float
    ) -> np.ndarray:
        """
    ...
    def apply_temporal_jitter(
        self, frames: np.ndarray, jitter_strength: float = 0.1
    ) -> np.ndarray:
        """
    ...
    def augment_video(self, video_path: str, aug_name: str, aug_config: dict) -> str:
        """
    ...
    def augment_dataset(self, input_dir: str, augmentations_to_apply: list = None):
        ...
        print(
            f"This will create approximately {len(video_files) * len(augmentations_to_apply)} new videos"
        )
        print("-" * 50)
                ...
                for aug_name in augmentations_to_apply:
                    ...
                    try:
                        ...
                        output_path = self.augment_video(
                            str(video_file), aug_name, aug_config
                        )
                        if output_path:
        ...
        temporal_augs = {
            "slow": {
                "spatial": None,
                "temporal": "slow",
                "description": "Slow motion (0.8x)",
            },
            "fast": {
                "spatial": None,
                "temporal": "fast",
                "description": "Fast motion (1.2x)",
            },
            "reverse": {
                "spatial": None,
                "temporal": "reverse",
                "description": "Temporal reverse",
            },
        }
