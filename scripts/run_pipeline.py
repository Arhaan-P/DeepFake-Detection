...
def stage_augment(videos_dir: str, output_dir: str):
    ...
    return run_command(
        [
            sys.executable,
            "scripts/preprocessing/augment_videos.py",
            "--input_dir",
            videos_dir,
            "--output_dir",
            output_dir,
...
def stage_preprocess(videos_dir: str, augmented_dir: str, output_file: str):
    ...
    cmd = [
        sys.executable,
        "scripts/preprocessing/preprocess_videos.py",
        "--videos_dir",
        videos_dir,
        "--output",
        output_file,
...
def stage_evaluate(
    checkpoint: str, features_file: str, enrolled_file: str, output_dir: str
):
...
def stage_demo(
    checkpoint: str, enrolled_file: str, video_path: str, claimed_identity: str
):
...
def full_pipeline(args):
    ...
    if not args.skip_preprocess:
        success = stage_preprocess(
            args.videos_dir, args.augmented_dir, args.features_file
        )
    ...
    if not args.skip_evaluate:
        checkpoint = os.path.join(
            args.output_dir, "checkpoints", "checkpoint_epoch_best.pth"
        )
...
Examples:
    ...
    # Run demo inference
    python run_pipeline.py --mode demo --video path/to/video.mp4 --claimed_identity "SubjectA"
        """,
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="full",
        choices=["full", "augment", "preprocess", "train", "evaluate", "demo"],
    ...
    # Demo arguments
    parser.add_argument(
        "--video", type=str, default=None, help="Video path for demo mode"
    )
    parser.add_argument(
        "--claimed_identity",
        type=str,
    ...
    # Skip stages
    parser.add_argument(
        "--skip_augment", action="store_true", help="Skip augmentation stage"
    )
    parser.add_argument(
        "--skip_preprocess", action="store_true", help="Skip preprocessing stage"
    )
    parser.add_argument("--skip_train", action="store_true", help="Skip training stage")
    parser.add_argument(
        "--skip_evaluate", action="store_true", help="Skip evaluation stage"
    )
    parser.add_argument(
        "--continue_on_error",
        action="store_true",
    ...
    if args.mode == "full":
        ...
        if not os.path.exists(args.features_file):
            ...
            print(
                "Please run preprocessing first: python run_pipeline.py --mode preprocess"
            )
            return
        stage_train(
            args.features_file,
