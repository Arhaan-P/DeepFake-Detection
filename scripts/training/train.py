...
def train_epoch(
    ...
    for batch_idx, batch in enumerate(train_loader):
        ...
        if (batch_idx + 1) % 50 == 0 or (batch_idx + 1) == len(train_loader):
            ...
            print(
                f"\r{progress:5.1f}% | Loss={loss_meter.avg:.4f} Acc={acc_meter.avg:.2f}%",
                end="",
                flush=True,
            )
    ...
    with torch.no_grad():
        ...
        print(
            f"  [Diag] P(auth) mean={sim.mean():.4f} std={sim.std():.4f} | "
...
def validate(model, val_loader, criterion, device, epoch, total_epochs):
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(val_loader):
            ...
            if (batch_idx + 1) % 10 == 0 or (batch_idx + 1) == len(val_loader):
                ...
                print(
                    f"\r{progress:5.1f}% | Loss={loss_meter.avg:.4f} Acc={acc:.2f}%",
                    end="",
                    flush=True,
                )
    ...
    if auth_mask.any() and fake_mask.any():
        print(
            f"  [Diag] Sim(auth)={all_similarities[auth_mask].mean():.4f} "
...
def train(args):
    ...
    print("  Scheduler: ReduceLROnPlateau (factor=0.5, patience=7)")
    ...
    for epoch in range(args.epochs):
        ...
        print(
            f"  Val Authentic F1: {val_metrics['authentic_f1']:.2f}% | "
...
def main():
    ...
    # Model
    parser.add_argument(
        "--multi_scale", action="store_true", help="Use multi-scale encoder"
    )
    ...
    parser.add_argument(
        "--num_workers", type=int, default=0, help="Data loader workers"
    )
