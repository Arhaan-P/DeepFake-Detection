...
class JointImportanceAnalyzer:
    ...
    def __init__(self, model: nn.Module):
        """
        Args:
            model: GaitDeepfakeDetector model to attribute predictions for
        """
        self.model = model
    ...
    def compute_joint_importance(
        ...
        attribution = (
            (video_features.abs().squeeze(0) * grad).detach().cpu().numpy()
        )  # (T, 78)
        ...
        coord_attr = attribution[
            :, FEATURE_GROUPS["coords"][0] : FEATURE_GROUPS["coords"][1]
        ]
        ...
        vel_attr = attribution[
            :, FEATURE_GROUPS["velocities"][0] : FEATURE_GROUPS["velocities"][1]
        ]
        ...
        angle_attr = attribution[
            :, FEATURE_GROUPS["angles"][0] : FEATURE_GROUPS["angles"][1]
        ]
...
def generate_explainability_report(
    ...
    return {"temporal_heatmap": temporal_heatmap, **importance}
