...
class GaitFeatureExtractor:
    ...
    GAIT_LANDMARKS = [
        11,
        12,  # Shoulders (for upper body sway)
        23,
        24,  # Hips
        25,
        26,  # Knees
    ...
    def compute_gait_features(self, pose_sequence: np.ndarray) -> Dict[str, np.ndarray]:
        ...
        gait_coords = np.array(
            [
                self.extract_gait_landmarks(pose)[:, :3] for pose in pose_sequence
            ]  # x, y, z only
        )  # Shape: (T, 12, 3)
        ...
        right_side = gait_coords_normalized[
            :, [1, 3, 5, 7, 9, 11], :
        ]  # Right landmarks
    ...
    def process_video(
        self, video_path: str, use_all_landmarks: bool = True
    ) -> Optional[Dict]:
        """
        ...
        if len(pose_sequence) < 10:
            print(
                f"Warning: Too few valid poses ({len(pose_sequence)}) in {video_path}"
            )
            return None
    ...
    def _normalize_sequence_length(self, sequence: np.ndarray) -> np.ndarray:
        ...
        for i in range(sequence_flat.shape[1]):
            interpolated[:, i] = np.interp(
                new_indices, old_indices, sequence_flat[:, i]
            )
...
def extract_features_from_dataset(
    input_dir: str, output_dir: str, sequence_length: int = 60
):
    """
    ...
    summary = {
        ...
        "feature_keys": (
            list(all_features[list(all_features.keys())[0]]["gait_features"].keys())
            if all_features
            else []
        ),
    }
...
if __name__ == "__main__":
    ...
    extract_features_from_dataset(
        input_dir=INPUT_DIR, output_dir=OUTPUT_DIR, sequence_length=SEQUENCE_LENGTH
    )
