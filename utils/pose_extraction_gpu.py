...
NOT used for any result reported in the paper or shipped checkpoints. Every
enrolled identity, training run, and evaluation in this repository uses
``pose_extraction.py`` (MediaPipe). This module exists for reference only,
is reachable solely via ``preprocess_videos.py --use_gpu``, and is
documented as discouraged in ``.github/instructions/rules.instructions.md``
("worse gait accuracy"). Do not use it to reproduce or extend published
results without re-running and re-reporting the full evaluation pipeline.
...
class MoveNetExtractor:
    ...
    def __init__(self, model_name: str = "thunder", sequence_length: int = 60):
        ...
        gpus = tf.config.list_physical_devices("GPU")
        print(
            f"✓ MoveNet {model_name} loaded (input size: {self.input_size}x{self.input_size})"
        )
    ...
    def preprocess_frame(self, frame: np.ndarray):
        ...
        img = tf.image.resize_with_pad(
            tf.expand_dims(frame, axis=0), self.input_size, self.input_size
        )
    ...
    def process_video(self, video_path: str) -> dict:
        ...
        if total_frames <= self.sequence_length:
            ...
            frame_indices = np.linspace(
                0, total_frames - 1, self.sequence_length, dtype=int
            )
        ...
        if len(pose_sequence) < self.sequence_length:
            ...
            pose_sequence = np.pad(
                pose_sequence, ((0, pad_length), (0, 0), (0, 0)), mode="edge"
            )
        else:
...
if __name__ == "__main__":
    ...
    test_video = "data/videos/SubjectA_F.mp4"
    if os.path.exists(test_video):
        ...
        if result:
            ...
            print(
                f"✓ Gait coords shape: {result['gait_features']['normalized_coords'].shape}"
            )
            print(
                f"✓ Joint angles shape: {result['gait_features']['joint_angles'].shape}"
            )
    else:
